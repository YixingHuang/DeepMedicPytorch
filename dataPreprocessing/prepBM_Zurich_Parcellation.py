...
def process(path, parce_path, has_label=True, n_channels=1):
    print(os.path.join(Path(path).parent, 'segment.nii.gz'))
    label = np.array(
            nib_load(os.path.join(Path(path).parent, 'segment.nii.gz')), dtype='float32', order='C')

    t1 =   np.array(nib_load(os.path.join(Path(path).parent, 'normalizedUKER.nii.gz')), dtype='float32', order='C')
...
def doit(dset):
    ...
    for sub, name in zip(subjects, names):
        ...
        # if idx < 161:
        #     continue
        process(path, parce_path, has_label, n_channels=len(modalities))


args.data_dir = '/mnt/data/zahra/data/train_parcellation'
args.test_data_dir = '/mnt/data/zahra/data/test_parcellation'
args.parcellation_train_dir = './BrainParcellation/HarvardOxford-sub/training/'
args.parcellation_test_dir = './BrainParcellation/HarvardOxford-sub/test/'
